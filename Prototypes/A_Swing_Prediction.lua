...
local function SafeInitMenu()
    ...
    local function ensureChargeOption(fieldName, defaultValue)
        if Menu.Charge[fieldName] == nil then
            if Menu.Misc[fieldName] ~= nil then
                Menu.Charge[fieldName] = Menu.Misc[fieldName]
            else
                Menu.Charge[fieldName] = defaultValue
            end
        end

        if Menu.Misc[fieldName] == nil then
            Menu.Misc[fieldName] = Menu.Charge[fieldName]
        end
    end

    ensureChargeOption("ChargeReach", true)
    ensureChargeOption("ChargeJump", true)
    ensureChargeOption("ChargeControl", false)
...
local function IsChargeReachEnabled()
    return (Menu.Charge and Menu.Charge.ChargeReach) or (Menu.Misc and Menu.Misc.ChargeReach) or false
end
...
local function PredictPlayer(player, t, d, simulateCharge, fixedAngles)
    ...
    local isChargeReachExploit = IsChargeReachEnabled() and player == pLocal and
...
local function OnCreateMove(pCmd)
    ...
    local isExploitReady = IsChargeReachEnabled() and hasFullCharge and isDemoman
    ...
    if isCurrentlyCharging then
        ...
        local isDoingExploit = IsChargeReachEnabled() and withinAttackWindow
    ...
    if can_attack then
        ...
        elseif Menu.Misc.InstantAttack and canInstantAttack and not Menu.Misc.WarpOnAttack then
            ...
            if pLocalClass == 4 and IsChargeReachEnabled() and chargeLeft == 100 and not attackStarted then
                attackStarted = true
                attackTickCount = 0
