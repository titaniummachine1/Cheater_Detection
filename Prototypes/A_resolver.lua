...
local function announceResolve(data)
	...
	data.lastYaw = yaw
	client.ChatPrintf(string.format("\x073475c9[Resolver] \x01Adjusted player \x073475c9'%s'\x01 yaw to \x07f22929%s",
		name, yaw))
end
...
local function playerShot(cmd, player)
	...
	if hasLocalPlayerFired(cmd) then
		return true
	end
...
local function handlePlayerShooting(cmd)
	...
	if playerDidShoot then
		...
		if victimInfo then
			...
			awaitingConfirmation[getSteamID(victim)] = {
				enemy = victim,
				hitTime = globals.CurTime() +
					getMinimumLatency(),
				wasHit = false
			}
		end
