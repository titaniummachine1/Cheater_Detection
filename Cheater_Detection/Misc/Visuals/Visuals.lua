...
local function UpdateTaggedCache()
	...
	local dirtyScore = DirtySystem.GetDirtyPlayers("score")
	local dirtyFlags = DirtySystem.GetDirtyPlayers("flags")

	-- Merge the two lists
	local dirtyPlayers = {}
	local seen = {}
	for _, id in ipairs(dirtyScore) do
		seen[id] = true
		dirtyPlayers[#dirtyPlayers + 1] = id
	end
	for _, id in ipairs(dirtyFlags) do
		if not seen[id] then
			dirtyPlayers[#dirtyPlayers + 1] = id
		end
	end
	...
	for _, id in ipairs(dirtyPlayers) do
		...
		DirtySystem.ClearDirty(id, "score")
		DirtySystem.ClearDirty(id, "flags")
	end
